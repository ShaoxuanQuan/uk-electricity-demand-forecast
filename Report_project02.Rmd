...
NESO (National Electricity System Operator) is in charge of managing and planning the electricity system in Great Britain (GB). To ensure the electricity supply meets demand at any time and offer long-term system planning, NESO needs to quantify uncertainty in peak demand many years ahead in order to avoid over- or under-building of the new infrastructure. Our objective is to construct a applicable linear regression model to forecast the future peak electricity demands everyday in Great Britain by using the given historical data for NESO.
...
<!--Clarify the modeling objectives, use M0 as the starting point for optimization, extract information from the data to refine the model, and conduct model validation-->
Overall, our Multiple Linear Regression model was developed through a step-by-step process of exploration and refinement. Starting from the baseline model, $M_0$, we systematically identified its shortcomings and improved it through feature engineering, variable selection, and data preprocessing to obtain the most satisfactory final model. The parameters for each model were estimated using the least squares method.

Our primary objective was to construct a linear regression model capable of predicting peak electricity demand based on given data. We first conducted a preliminary analysis of the data and converted variables **wdayindex**, **monthindex**, **start_year** and **DSN**  from numerical to categorical, as their nature is inherently categorical. Then we tested the baseline model, $M_0$, using the initially preprocessed data. The results indicated poor performance across multiple metrics, such as $R^2$. Through data analysis, we identified potential data issues and revised the variable selection, and then explored ways to enhance model performance by ensuring all relevant data contributed effectively. Step-by-step, we first replace variable **temp** with variable **TE** and add variables **start_year** and **DSN**. Then we modified **solar_S**. Subsequently, based on domain insights, we adjusted TE, forming Model $M_1$. Finally, we introduced interaction terms and higher-order terms, resulting in Model $M_2$.
...
<!--Evaluate the performance of the final model and the insights it provides, such as identifying key variables that significantly impact demand and predicting future demand trends. Assess the model’s practical value, including confidence intervals, its role in decision-making, and discuss potential future work-->
...
Based on the model results, we conclude that the conditions of solar and the temperature have significant impact on peak demand prediction. This suggests that NESO should prioritize improving the monitoring and accuracy of these variable. In the future, if the number of solar become greater, the peak of electricity demand will grow, while if the temperature grows in the future, the peak of electricty demand will drop. Moreover, since electricity demand is influenced by numerous external factors, and our model follows a linear regression framework, it remains flexible for future enhancements—allowing the incorporation of new predictive variables as they are identified.
...
In order to analysis whether data **demand_gross** has some periodic fluctuations,we use **wdayindex** as the horizontal axis and peak electricity **demand_gross** as the vertical axis, we construct a boxplot (Figure 1).
...
From Figure 1, we observe that the peak electricity demand on Monday through Thursday is nearly identical, with a maximum value of approximately 60,000 MW and a lower bound of the normal range slightly above 40,000 MW. The mean demand for these days is around 5,500 MW. A few outliers deviate significantly from the overall trend, likely due to overlaps with holidays, which may have led to an unexpected drop in electricity demand on what would typically be a high-demand workday.
...
Variables **wdayindex**, **monthindex**, **year**, **start_year**, **DSN** are main temporal indicators and we use. We use them to figure out the impact of "year effect" on electricity demand.Variable **wdayindex** is the day of week,beginning from Sunday(0). Variable **monthindex** is the month of year, begining with January(0). Variable **start_year** represents the year at the start of the winter period. Variable **DSN** is the number of days since the 1st of November. 

Among these five time-related variables, share a common characteristic: unlike the numerical temperature-related data mentioned earlier, they are inherently categorical. Therefore, they cannot be directly treated as numerical variables. Data types **wdayindex** and **monthindex**，**year** only indicate the day of the week and month and year in which the data appear, making them entirely independent. In contrast, data types **start_year** and  **DSN** are interrelated with previous data and exhibit temporal dependencies. 

Besides, since each cycle of **DSN** (starting on November 1st and ending on March 31st of the following year) fully encompasses each cycle of **monthindex** (which follows the same period) while providing a more granular representation, we will use **DSN** data exclusively in the subsequent analysis. Apart from the initially given model, **monthindex** data will no longer be used. <!--The two variables are significantly correlated, as indicated by the chi-square test. Use box plots to compare trends, highlighting differences. Prioritize data that retains the most information-->
...
## Data Preprocessing
<!--Data cleaning to ensure no missing values or invalid entries and select relevant data based on model requirements；-->
We first performed data cleaning; however, the dataset is complete with no missing values. Through preliminary analysis, we determined that data **wdayindex** , **monthindex** , **start_year** cannot be used directly, as they represent the days of the week, the months and the years by categorical numbers without intrinsic ordinal significance. For instance, Monday and Tuesday do not inherently differ in their effect on demand, yet treating them as numerical values (e.g., 1 and 2) could introduce misleading patterns into the model. To address this, we transform these categorical variables into a structured format that eliminates spurious numerical interpretations while preserving their relevant information. Then **wdayindex** are categorized into seven groups, denote by 0 to 6 respectively. Similarly **monthindex** are categorized into five groups, denote by 0,1,2,3,10,11. Also, **start_year** are categorized into 23 groups, denote from 1991 to 2013. Accordingly, after updating the data and refining the variable selection, we fitted the model and obtained Model $M_0$. However the performance of Model $M_0$ is very poor.

<!--The second model to process"solar_S"--><!--we add some plot to proof this change-->
...
### Structure and Relevance of the "SCS_hourly_temperature" Dataset(Goal5)
  <!--The third model to process"TE"-->
<!--determine which period in a day is more suitable to model & make a plot to explain this fluctuation for  simulation）-->
...
<!--We used a heatmap or table to show that the original solar_S performed poorly in the new model, even though it was an important variable. To preserve its prior influence, we processed and modified the solar_S data. After reintroducing it into the model, performance improved. The concept of exchangeability can help justify that the modified model holds practical significance-->
$$
...
- \(\beta_10 \cdot \text{temp}_i \times \text{start_year}_i\):The interaction term between temperature and start_year. This term demonstrates models how the effect of temperature on demand varies over different years.
...
### Comparison Metrics
When evaluating and comparing the model's fitting results, we consider three metrics: adjusted $R^2$, MSE and BIC.

**Adjusted** **$R^2$**: In regression analysis, the coefficient of determination $R^2$ quantifies how well the independent variables explain the variation in the dependent variable. It is defined as:
$$
R^2 = 1- \frac{\sum_{i=1}^n(y_i-\hat{y_i})^2}{\sum_{i=1}^n(y_i-\bar{y_i})^2},
$$
where $y_i$ represents the observed values, $\hat{y_i}$ denotes the predicted values, and $\bar{y}$ is the mean of the dependent variable. However one major limitation of $R^2$ is that it tends to increase when additional predictors are added, even if those predictors contribute little to the model's explanatory power, especially in Multiple Linear Regression model. To address this issue, the **Adjusted** **$R^2$** metric is introduced, which accounts for model complexity by incorporating degrees of freedom. The **Adjusted** **$R^2$** is computed as follows:
$$
R_{adj}^2 = 1-(\frac{(1-R^2)(n-1)}{n-p-1}),
$$
where $R_{adj}^2$ is the adjusted coefficient of determination, $R^2$ is the standard coefficient of determination, $n$ is the number of observations and $p$ is the number of predictors in the model. $R_{adj}^2$ will increase only if new added variables contribute significantly to the model, otherwise it may decrease, thus discouraging overfitting. It is a refined measure that provides a more reliable evaluation of a model's predictive capability.

**$MSE$**(Mean Squared Error): The **$MSE$** is a widely used error metric in statistics, assessing the average squared deviation of predicted values from the actual values. It essentially represent the expected squared error, providing and overall measure of model fit. Given a dataset with $n$ observations $(y_i,\hat{y_i})$, where $y_i$ denotes the actual values and $\hat{y_i}$ represents the predicted values, the **$MSE$** is defined as:
$$
MSE = \frac{1}{n} \sum_{i=1}^{n}(y_i-\hat{y_i})^2.
$$
**$MSE$** quantifies the average squared deviation between predictions and ground truth. A smaller **$MSE$** indicates better predictive performance. Due to the square term, larger errors contribute more to the overall metric, making **$MSE$** sensitive to outliers. **$MSE$** captures both variance and bias of an estimator, following the decomposition:
$$
MSE(\hat{\beta}) = Var{\hat{\beta}}+(\mathbb{E}[\hat\beta]-\beta)^2,
$$
where $\beta$ is the true parameter, and $\hat{\beta}$ is the estimate. This formulation helps assess estimator accuracy. **$MSE$** remains a fundamental metric in model evaluation, particularly in regression tasks, where it serves as a benchmark for comparing different predictive approaches.
...
### Comparation With M0 Model(Goal1)
...
<!--show the result，explain each criterion meaning-->
...
<!--Is the prediction accuracy the same across all months? or is the model better at predicting specific months?-->
...
1.Comparison of Errors Across Months: Months with higher RMSE (e.g., January (0), March (2), and December (11)) indicate that the model has larger prediction errors during these periods. Months with lower RMSE (e.g., August (7) and November (10)) suggest that the model performs more accurately during these times.

2.Impact of Confidence Intervals: Wider confidence intervals (e.g., in March (2) and December (11)) indicate higher uncertainty in predictions, possibly due to greater variability in data or poorer model adaptation to these months. Narrower confidence intervals (e.g., in August (7) and November (10)) suggest more stable and reliable predictions with less variance.

3.Seasonal Effects: January (winter) and August (summer) are typically peak electricity demand periods, but the RMSE trend shows that errors are higher in January while lower in August. This may indicate that the model struggles more with winter demand prediction. During transitional seasons (e.g., October (10)), prediction errors are relatively low, suggesting that the model performs better under moderate climate conditions.
...
monthly_rmse_analysis <- function(data, model, conf_level = 0.95) {
  ...
  # Create RMSE plot
  rmse_plot <- ggplot(rmse_results, aes(x = factor(monthindex), y = rmse)) +
...
According to box plot(Figure 9), to evaluate the prediction accuracy across different months, we can compare the actual (blue) vs. predicted (red) demand distributions in the box plot: the model's prediction accuracy varies slightly by month. It performs better in October and November but less accurately in February and January, possibly due to more fluctuations or external factors affecting demand in these months. 

The model performs well in most months, as the median (black line inside each box) of the predicted and actual values are closely aligned. The interquartile ranges (IQRs) of the predicted and actual values also overlap significantly, indicating that the overall distribution of predictions is similar to the actual demand. November and October exhibit the highest accuracy. The predicted and actual distributions are nearly identical in terms of median, spread, and whiskers.This suggests that the model effectively captures seasonal patterns in these months. February appears to have the largest discrepancy between actual and predicted demand. The median predicted value is slightly lower than the actual median, and the spread of actual demand is slightly wider than the predicted values. Additionally, January also shows some deviation, particularly in the lower tail, where the actual demand has more extreme low values compared to predictions.
...
<!--Conclusion of performance of model-->
<!--Based on the estimated beta coefficients, we first summarize the direction of association between each factor and demand. We then perform a hypothetical analysis to explore how future changes in key variables (e.g., increases or decreases) might influence demand-->
...
<!--We assess model stability through bootstrap sampling. If the estimated parameters remain relatively consistent across resampled datasets, this suggests that the model is robust and not overly sensitive to data variation-->
...
<!--interpret the estimated β coefficients to understand the direction and strength of each factor's influence on demand. Based on this, we conduct a scenario-based discussion: for instance, if temperature or solar radiation were to increase or decrease in the future, we examine how such changes might affect overall demand-->
<!--The analysis suggests that maximum electricity demand is typically higher during working hours than on rest days. Consequently, it would be reasonable for NESO to adjust and potentially reduce electricity supply during holidays or non-working periods-->
...
## Model Assumptions
<!--1.The error terms are linearly independent and follow a normal distribution-->
1.Assume that the error terms are linearly independent and follow a normal distribution. This assumption is crucial for establishing a multiple linear regression model, as it ensures the validity of inferential statistics, such as hypothesis testing and confidence intervals, and facilitates the estimation of model parameters.

<!--2.The data is assumed to be stationary-->
2.Assume that the data **solar_S**, **wind**,etc. are stationary. This assumption is fundamental when constructing a multiple linear regression model, as stationarity ensures that the statistical properties of the data, such as mean and variance, do not change over time. This consistency allows for reliable model estimation and valid inference, as non-stationary data can lead to spurious relationships and biased parameter estimates.

<!--3.6:00 PM is identified as the peak time for electricity demand-->
3.Assume that the peak electricity demand occurs at 6 PM. This time is characterized by the highest consumption levels, which is crucial for predicting daily electricity demand patterns. By incorporating this peak time as a key feature in the multiple linear regression model, we can improve the accuracy of forecasts for future electricity demand, accounting for daily fluctuations and identifying the factors that influence peak usage.

<!--4.All variables are assumed to be mutually independent-->
4.Assume that the independence of all variables is a key assumption in constructing the multiple linear regression model to predict the daily peak electricity demand. This assumption simplifies the model by treating each predictor as having no correlation with the others, allowing for clearer interpretation of their individual contributions to the prediction of peak demand. However, it is important to note that while this assumption aids in model development, it may not always hold true in real-world scenarios where variables can exhibit interdependencies.


## The limitations of the Model(Goal6)
<!--What would be the limitations of your model?-->
1.One key limitation of the model is the assumption that the error terms are linearly independent and follow a normal distribution. In practice, this assumption may not always hold. Factors such as unobserved variables, temporal dependencies, or heteroscedasticity can introduce correlations among errors or cause deviations from normality. Violations of these assumptions can impact the validity of statistical inferences, potentially leading to biased parameter estimates and unreliable confidence intervals. Therefore, careful diagnostic checks and robustness tests are necessary to assess the impact of these assumptions on the model's performance.
...
3.One key limitation of the regression model is its assumption of a fixed relationship between the independent and dependent variables. This model typically overlooks the temporal dependencies that may exist between data points. It assumes that the input features are independent and identically distributed (i.i.d.), meaning there is no direct time-series dependence between individual data points. As a result, the model may not adequately capture time-related patterns or trends in the data, which could lead to suboptimal performance, especially when applied to time-dependent or sequential data such as electricity demand. This limitation restricts the model's ability to account for the dynamic nature of the underlying processes.

4.A significant limitation of the regression model is its inability to handle seasonality or trends inherently. Even when time is introduced as an independent variable, it is typically treated as a simple linear term or an external feature, without the capability to automatically capture seasonal fluctuations or long-term trends in the data. This limitation of the regression model may reduce its effectiveness in modeling time-dependent data with complex patterns.

5.Regression models (such as linear regression, ridge regression, and Lasso regression) are relatively simple and easy to understand, but they lack the flexibility to effectively model complex time series structures, such as long cycles or seasonal variations. For intricate nonlinear relationships or non-stationary data, these models may struggle to provide accurate predictions. As a result, their applicability may be limited when dealing with data that exhibits intricate temporal dependencies or patterns that cannot be captured by linear assumptions.

To refine the limitations above, time series models, such as ARIMA, SARIMA, and Exponential Smoothing, are specifically designed to handle temporal dependencies and patterns within data. Unlike regression models, these models are capable of capturing trends, seasonality, and autocorrelation, making them better suited for forecasting in contexts where such patterns are prevalent. Given the nature of electricity demand, which often exhibits strong seasonal fluctuations and long-term trends, time series models may provide more accurate predictions by effectively modeling these inherent temporal structures. Consequently, such models are likely more appropriate for this task, offering a better fit for the data's dynamic nature.

## How the Model Supports NESO's Long-Term Planning Goals
1.Quantifying Uncertainty in Peak Demand: By generating probabilistic forecasts, our model provides a rigorous assessment of uncertainty in peak demand estimates. This enables NESO (National Grid ESO) to evaluate the probability of various demand scenarios, ensuring more informed decision-making in infrastructure planning. By accounting for potential fluctuations, the model helps mitigate the risks of both over- and under-building, ultimately enhancing the reliability and efficiency of long-term energy supply management.

2.Optimizing Infrastructure Investment: By forecasting peak electricity demand over extended time horizons, our model provides NESO with critical insights into when and where new infrastructure investments, such as power plants and transmission lines, are most needed. This data-driven approach enhances the efficiency of long-term planning, ensuring that financial and operational resources are strategically allocated to regions with the highest demand growth. By proactively addressing future capacity needs, the model supports a more resilient and cost-effective energy infrastructure.

3.Supporting Renewable Energy Integration: As the energy grid increasingly relies on renewable sources, which are inherently variable, accurate demand forecasting becomes essential for grid stability. Our model helps predict periods of peak and low electricity demand, aligning them with fluctuations in renewable energy generation. This enables NESO to proactively manage grid reliability by optimizing backup generation, energy storage deployment, and demand-side response strategies. By improving the coordination between demand patterns and renewable supply, the model supports a more efficient and sustainable energy transition.

4.Enhancing Risk Management: The model's capability to predict peak demand and quantify its associated uncertainty enables NESO to develop more effective risk management strategies. By providing probabilistic forecasts, the model helps anticipate potential demand surges and assess the likelihood of extreme scenarios. This allows for better-informed decisions on backup energy sources, load balancing measures, and contingency planning to ensure grid stability. Additionally, improved risk assessment supports proactive policy adjustments, reducing the likelihood of supply shortages or costly emergency responses.
